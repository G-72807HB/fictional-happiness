...
/* IBankung X GO
Lightweight Internet-Banking System Backed By GO
Client-Edition Release
...
NIM : 1301194150
II.
Nama : MW. Fathurrahman
NIM : 1301190407


Features list As Of Before 30.11.2019 23:59 :
...
// N Denotes Constraint for Fixed Array
const N = 20192020
...
var (
	...
	db []nasabah = []nasabah{{qidNasabah{"DAP", "FAR", 1000000}, []transaksi{{"011119", 1000000, 0, 1000000, "StarterPack"}}}}

	dbTemp []byte = []byte("DAP FAR 1000000 011119 1000000 0 1000000 StarterPack")
)

// Reinitialization
// Retrun Content To Ther Default Values

func reIns() {

	//HTML Parts

	deHeder = Header{"IBanking X GO"}
	...
	deMenu = []Menu{{"Landing", "/index/"}, {"Dashboard", "/dashb/"}, {"About", "/about/"}, {"Sign Out", "/logout/"}}

	Foo = Footer{"Copyright 2019 - All Rights Reserved"}
	...
	if exist && !userExist(new.Username) {
		...
		if err == nil {
			...
			if write != nil {
				...
				data.Body.Content = []Content{{"Error: ", "Database Error"}}
		...
		data.Body.Content = []Content{{"Error: ", "Couldn't Connect To Database"}}

		return false
	}

	data.Body.Content = []Content{{"Error: ", "Data Is A Duplicate"}}
...
func newTransaction(n, ref, sum string) bool {
		...
		if val.ID.Username == n {
			...
			if ref == "CCH1A4" {
				...
				if saldo < 50000 {
					...
					data.Body.Content = []Content{{"Error: ", "Not Enough Balance Left"}}
			...
			if !err {
				...
				data.Body.Content = []Content{{"Error: ", "No Such User Exist"}}
			...
			if ref == "CCH1A4" {
				...
				if err == nil {
					...
					if userExist(ref) {
						...
						if newTransaction(ref, "CCH1A4", sum) {

							return true
						}

						data.Body.Content = []Content{{"Error: ", "Transaction Failed"}}
	...
	data.Body.Content = []Content{{"Error: ", "Unknown Error"}}
...
func sessChk(w http.ResponseWriter, r *http.Request) {
	...
	if userExist(auth) {
		...
		deMenu = []Menu{{"Landing", "/index/?auth=" + auth}, {"Dashboard", "/dashb/?auth=" + auth}, {"About", "/about/?auth=" + auth}, {"Sign Out", "/logout/"}}
	} else {
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	data = Page{Head{"Sign In", deCSS}, Body{deHeder, deMenu, []Content{{"", ""}}, Foo}, fetchData("")}
	...
	if r.FormValue("SignIn") == "Sign In" && r.FormValue("SignUp") == "" {
		...
		if !strings.Contains(pass, " ") && !strings.Contains(key, " ") {
			...
			if userChk(pass, key) {
				...
				data.Body.Content = []Content{{"Error: ", "Username and/or Password Do Not Match"}}
			}
		} else {

			data.Body.Content = []Content{{"Error: ", "Username and/or Password Cannot Contain Whitespace"}}
		}
		...
		if !strings.Contains(pass, " ") && !strings.Contains(key, " ") {
			...
			if createUser(pass, key) {
				...
				data.Body.Content = []Content{{"Error: ", "User already exist"}}
			}
		} else {
...
func idHandler(w http.ResponseWriter, r *http.Request) {
	...
	data = Page{Head{"Landing", deCSS}, Body{deHeder, deMenu, []Content{{"", ""}}, Foo}, fetchData(auth)}
...
func dashHandler(w http.ResponseWriter, r *http.Request) {
	...
	data = Page{Head{"Dashboard", deCSS}, Body{deHeder, deMenu, []Content{{"", ""}}, Foo}, fetchData(auth)}
	...
	if r.FormValue("Update") == "Update Account" {
		...
		if !strings.Contains(r.FormValue("newUsername"), " ") || !strings.Contains(r.FormValue("newPassword"), " ") {
			...
			data.Body.Content = []Content{{"Error: ", "Referal Code Cannot Contain Whitespace"}}
		}
	}
	...
	if r.FormValue("Transaction") == "GO!" {
		...
		if !strings.Contains(r.FormValue("Code"), " ") {
			...
			if newTransaction(auth, r.FormValue("Code"), r.FormValue("Jumlah")) {
				...
				data.Body.Content = []Content{{"Great: ", "Transaction Success!"}}
			} else {
	...
	if r.FormValue("Search") == "Search" {
		...
		if len(data.Data.Riwayat) > 0 {
			...
			data.Body.Content = []Content{{"Found: ", "Showing All Results Of " + fmt.Sprintf("%q", key)}}
		} else {
			...
			data.Body.Content = []Content{{"Not Found: ", "No Such Data"}}
		}
	}
	...
	if r.FormValue("Date") != "" || r.FormValue("Debit") != "" || r.FormValue("Credit") != "" || r.FormValue("Balance") != "" || r.FormValue("Notes") != "" {
		...
		data.Body.Content = []Content{{"Yeay: ", "Data Is Now Sorted"}}
...
func aboutHandler(w http.ResponseWriter, r *http.Request) {
	...
	data = Page{Head{"About", deCSS}, Body{deHeder, deMenu, []Content{{"", ""}}, Foo}, fetchData(auth)}
...
func main() {
	...
	if (launchBrowser) == nil {
		...
		fmt.Printf("\nServer Status: On Air!\n\n\a")
	} else {

		fmt.Printf("\nTry Opening http://127.0.0.1:8080/index/ or http://localhost:8080/index/\n\a")
	}
